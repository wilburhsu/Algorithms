...
对于`volatile`变量的写操作，Java虚拟机会在该操作之前插入一个释放屏障（Release Barrier），并在该操作之后插入一个存储屏障（Store Barrier）
...
对于`volatile`变量的读操作，Java虚拟机会在该操作之前插入一个加载屏障（Load Barrier），并在该操作之后插入一个获取屏障（Acquire Barrier）
