...
1. 判断是否需要扩容 
2. key值计算hash，得到32位hashcode 
3. 将得到的hashcode右移16位取得高16位 
4. 将前两步得到的值进行异或运算得到最终的hash值 
5. 然后根据数组长度进行取模计算， `(n - 1) & hash` ，得到在数组中的 index 下标 
6. 索引位置为空，建立新的k-v节点；如果不为空，先判断已存在的key值的hash与要插入的是否相同，相同覆盖，不相同以**链表**形式连接到当前key值之后；若链表长度大于8且数组长度大于64，将链表转换为红黑树，否则对数组进行扩容
### ConcurrentHashMap
ConcurrentHashMap是Java中一个线程安全的哈希表，它可以在多线程环境下高效地进行读写操作。它的实现原理在JDK1.7和JDK1.8之间有所不同，主要区别如下：

- JDK1.7中，ConcurrentHashMap采用了分段锁的技术，将哈希表分为若干个Segment，每个Segment相当于一个小的哈希表，有自己的锁。这样，多个线程可以同时对不同的Segment进行修改，提高了并发性能。但是，这也导致了两次哈希定位的过程，以及Segment数组的额外开销。
...
### Java中的锁
Java锁是一种机制，用于控制多个线程对共享资源的访问。Java锁机制可以分为悲观锁和乐观锁两种，也可以按照锁的实现方式分为显式锁和隐式锁，以及其它不同的类型和特性。
**按照锁的实现方式分类：**

1.  内置锁：`synchronized`关键字 
2.  显式锁：`java.util.concurrent.locks`包下常用的类
   1. `Lock`接口
`ReentrantLock`：可重入锁，`Lock`接口的一个实现，继承AQS，并且提供了可重入、公平和非公平等特性
   2. `ReadWriteLock`接口
`ReentrantReadWriteLock`：可重入的读写锁，实现了`ReadWriteLock`接口。最主要的有两个方法：`readLock()`和`writeLock()`用来获取读锁和写锁。它提供了读写分离的功能。读写锁允许多个线程同时获取读锁，但只允许一个线程获取写锁，并且读写互斥
...
2. 自旋锁：一种在等待获取互斥量时不会放弃处理器执行时间而进行忙循环（自旋）等待的机制，在多核处理器系统中可以避免线程切换带来的开销
3. 偏向锁：一种优化synchronized内置锁的机制
...
要想解决死锁问题，那就需要打破上述四个条件中的任意一个或多个。
解决思路有：
...
- 改变获取锁的顺序，使所有线程按照相同的顺序获取锁，避免循环等待。
- 使用信号量或超时机制，在获取锁时设置一个超时时间，在超时时间内没有获取到锁则放弃并释放已经获取到的锁。
- 使用jvisualvm/jmc/jconsole等工具检测和分析死锁，并手动解除死锁状态。
### AQS
...
##### G1收集器
G1（Garbage First）是 JDK 9 中引入的垃圾回收器，它是一种分代垃圾回收器，也是基于标记-整理算法的。
G1 的核心思想是将整个堆内存分成多个大小相等的区域，每个区域被称为一个 Region。G1 通过将多个 Region 组成不同的集合，分为 Eden 区、Survivor 区、Humongous 区和 Old 区等。其中，Eden 区和 Survivor 区用于年轻代的垃圾回收，Humongous 区用于存放大对象，Old 区用于老年代的垃圾回收。
G1 的垃圾回收过程分为以下几个步骤：

- 初始标记：标记所有 GC Roots 直接引用的对象，速度较快；
- 并发标记：从 GC Roots 直接或间接引用的对象开始，标记所有存活对象；
- 最终标记：标记所有在并发标记期间产生的新对象；
- 筛选回收：根据 Region 的回收价值和空间情况，选择回收价值最高的 Region 进行垃圾回收。

G1 的优点是具有高效的回收性能和可预测的停顿时间，可以避免长时间的 Full GC 停顿。
#### GC调优
