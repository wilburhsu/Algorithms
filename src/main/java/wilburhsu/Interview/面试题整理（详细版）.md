...
详细的解释：[https://www.cnblogs.com/skywang12345/p/3324958.html](https://www.cnblogs.com/skywang12345/p/3324958.html)
...
广义上的多态：[https://www.runoob.com/java/java-polymorphism.html](https://www.runoob.com/java/java-polymorphism.html)
#### 重载和重写的区别
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/29736809/1685263614486-528335ec-5f9b-47bb-902f-584823a82f11.png#averageHue=%23767575&clientId=ua5974e6f-4055-4&from=paste&height=243&id=ud7693bb0&originHeight=486&originWidth=1878&originalType=binary&ratio=2&rotation=0&showTitle=false&size=95450&status=done&style=none&taskId=u5be9d445-9666-466d-8691-4a456b13a55&title=&width=939)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/29736809/1685263642074-7c9bb5d8-56aa-419a-b0c1-640e5aaf1319.png#averageHue=%23ededed&clientId=ua5974e6f-4055-4&from=paste&height=299&id=u04d16fd5&originHeight=598&originWidth=1878&originalType=binary&ratio=2&rotation=0&showTitle=false&size=114256&status=done&style=none&taskId=uca90cd36-5882-4b89-9400-bd80c9b2050&title=&width=939)
...
JDK1.8中，ConcurrentHashMap放弃了Segment的结构，而是采用了CAS无锁算法和红黑树的优化，降低了锁的粒度和竞争。具体来说，JDK1.8中的ConcurrentHashMap使用了一个Node数组作为哈希桶，每个Node节点包含了键值对和一个指向下一个节点的引用。当发生哈希冲突时，会形成一个链表或者树结构。为了保证线程安全，ConcurrentHashMap对Node节点的值和引用都使用了volatile修饰符，保证了内存可见性。同时，当需要修改Node节点时，会使用CAS操作来比较并替换旧值，避免了加锁的开销。当链表的长度超过一定阈值时，会转换为红黑树结构，提高了查找效率
## 2 Java高级
### 2.1 I/O
#### 如何理解I/O
...
##### JMM的四种读写屏障
由于物理世界中的CPU屏障指令和效果各不一样，为了实现跨平台的效果，针对读操作load和写操作store，Java在JMM内存模型里提出了针对这两个操作的四种组合来覆盖读写的所有情况，即：读读`LoadLoad`屏障、读写`LoadStore`屏障、写写`StoreStore`屏障、写读`StoreLoad`屏障。这些屏障可统一用XY来表示，XY屏障的作用是禁止屏障左侧的任何X操作与屏障右侧的任何Y操作之间进行重排序。
> **LoadLoad屏障：**
...
在使用CountDownLatch时，需要注意以下几个事项：
1. CountDownLatch的计数器不能重复使用：CountDownLatch的计数器只能通过构造函数传入初始值，在运行期间不能再进行增加操作，因此，一旦CountDownLatch的计数器被减到0，就不能再用来进行等待操作了。如果需要动态地增加计数器的值，进行多次等待，可以使用CyclicBarrier或者Semaphore。
2. 确保等待线程能够被唤醒：在使用CountDownLatch时，需要确保计数器能够减到0，从而唤醒所有等待线程，否则会导致等待线程永远无法被唤醒，从而引起死锁
    - CountDownLatch的计数器应该小于等于等待线程的数量；
    - countDownLatch.countDown();这一句话尽量写在finally中，或是保证此行代码前的逻辑正常运行，避免出现异常会导致无法countDown
3. CountDownLatch的性能：在使用CountDownLatch时，需要注意它的性能问题。由于CountDownLatch是通过Unsafe.park()和Unsafe.unpark()来实现线程等待和唤醒的，因此在高并发的场景下，可能会引起性能问题，从而影响系统的稳定性和可靠性。因此，在使用CountDownLatch时，需要仔细评估其性能影响，并根据具体情况进行优化。
